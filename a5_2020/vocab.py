...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents [['this', 'is, 'the', 'first', 'sentence'], ['this', 'is', 'the', 'second', 'one']]
        len(sents) is the sent_batch_size
        sents after padding is [sent_batch_size, m_sent, m_word]
        output is [m_sent, sent_batch_size, m_word]
        """
        ...
        t = torch.tensor(
            pad_sents_char(self.words2charindices(sents), self.char_pad),
            dtype=torch.int64,
            device=device
        ).transpose(0,1)
    ...
    def to_input_tensor(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_var = torch.tensor(sents_t, dtype=torch.int64, device=device)
