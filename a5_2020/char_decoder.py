...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        # x_emb [length, batch, e_char]
        out, (h_out, c_out) = self.charDecoder(x_emb, dec_hidden)
        # out [length, batch_size, hidden_size]
        scores = self.char_output_projection(out)
        # scores [length, batch_size, vocab_size]
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        # char_sequence [length, batch_size]
        input = char_sequence[:-1,]
        target = char_sequence[1:,]

        # input/target [length-1, batch_size]
        scores, dec_hidden = self.forward(input, dec_hidden)

        # scores [length-1, batch_size, vocab_size]
        scores = scores.permute(1,2,0)
        target = target.permute(1,0)

        # scores [batch_size, vocab_size, length-1]
        # target [batch_size, length-1]
        loss = nn.functional.cross_entropy(scores, target, reduction='sum', ignore_index=self.target_vocab.char_pad)
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        current_char = torch.full(
            size=(1,batch_size),
            fill_value=self.target_vocab.start_of_word,
            device=device,
            dtype=torch.int64,
        )

        for _ in range(max_length):
            # current_char [length=1, batch_size]
            scores, initialStates = self.forward(current_char, initialStates)
            # scores [length=1, batch_size, vocab_size]
            p = nn.functional.softmax(scores, dim=2)
            # p [length=1, batch_size, vocab_size]
            current_char = p.argmax(dim=2)
            # current_char [length=1, batch_size]
            output_word.append(current_char)

        output_word = torch.cat(output_word).t().numpy()

        decodedWords = []

        # output_word [batch_size, length]
        for row in output_word:
            sent = ''

            for e in row:
                if e == self.target_vocab.end_of_word:
                    break

                sent += self.target_vocab.id2char[e]

            decodedWords.append(sent)
